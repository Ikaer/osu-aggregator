...
if (nconf.get('c') === true) {
    ...
    mongoose.connect(nconf.get('mongodbPath'), function (err) {
        if (err) throw err;
        console.log("%s has been launched.", configFilePath)
...
//require('longjohn');
    ...
    var processesConfig = require('nconf');
    processesConfig.file('config', 'config/config.json');

    var debugPort = 30000;
    _.each(processesConfig.get('configs'), function (c, i) {
        debugPort++;
        var ls = childprocess.fork('app.js', ['--c', '--config "config/' + c + '.json"', "-debug"]);
